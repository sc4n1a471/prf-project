...
async function getPassesInUse(req: Request, res: Response) {
	if (req.isAuthenticated()) {
		...
		if (isAdmin) {
			PassInUse.find({ userId: req.user, isActive: true })
				.populate("pass")
				.populate("user")
				.exec()
				.then((data) => {
			...
			PassInUse.find({ payerId: req.user, isActive: true })
				.populate("pass")
				.populate("user")
				.exec()
				.then((data) => {
...
async function getPassInUse(req: Request, res: Response) {
	if (req.isAuthenticated()) {
		PassInUse.findById(req.params.id)
			.populate("pass")
			.exec()
			.then((data) => {
				res.status(200).send(data)
...
async function checkPassInUseValidity(
	passInUse: IPassInUse,
	serviceId: mongoose.Schema.Types.ObjectId
): Promise<boolean> {
	let now = new Date()
	...
	if (!passInUse.pass.occasionLimit) {
		...
		if (passInUse.validUntil < now || passInUse.occasions >= passInUse.pass.occasionLimit) {
			console.log(
				`PassInUse is expired or reached its occasion limit: ${passInUse.validUntil}, ${passInUse.occasions}, invalidating...`
			)
			await invalidatePassInUse(passInUse._id)
			return false
		}
		console.log(
			`PassInUse is valid and has ${passInUse.pass.occasionLimit - passInUse.occasions - 1} occasions left.`
		)
	}
